...
const validatePhoneNumber = (phoneNumber) => {
  let pattern = /^((\(\d{3}\))|\d{3})+(\s|-)?\d{3}(\s|-)?\d{4}$/;
...
const findTagNames = elements => {
  let pattern = /\/\w+/g;
  let strung = elements.join('');
  return strung.match(pattern);
};
