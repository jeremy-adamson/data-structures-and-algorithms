...
const addTwo = (arr) => {
  let newArray = [];
  arr.forEach(value => newArray.push(value + 2));
  return newArray;
};
...
const getStatName = (arr, minBaseStat) => {
  let newArray = [];
  arr.filter(value => value.baseStat > minBaseStat).forEach(value => newArray.push(value.stat.name));
  return newArray;
};
...
const getCharactersWithoutChildren = (arr) => {
  let newArray = arr.filter(value => value.children === null || value.children === undefined);
  return newArray;
};
