...
const maxInArray = (arr) => {
  let max = arr.reduce((accum, value) => Math.max(accum, value));
  return max;
};
...
const hasChildrenValues = (arr, character) => {
  let children = arr.find(person => person.name === character).children;
  if (children && Object.values(children)){
    return true;
  }
  else {
    return false;
...
const hasChildrenEntries = (arr, character) => {
  let person = arr.find(person => person.name === character);
  if (!person){
    return false;
  }
  let childField = Object.entries(person).find(element => element[0] === 'children');
...
const totalCharacters = (arr) => {
  let total = arr.reduce((accum, person) => {
    accum++;
    if (person.spouse){
      accum++;
    }
    if (person.children && person.children.length > 0){
      accum += person.children.length;
    }
    return accum;
  }, 0);
  return total;
...
const houseSize = (arr) => {
  ...
  for (let person of arr){
    ...
    sizes.push({ 'house': person.house, 'members': total});
  }
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  for (let person of arr){
    ...
    survivors.push({ 'house': person.house, 'members': total});
  }
  return survivors;
};
