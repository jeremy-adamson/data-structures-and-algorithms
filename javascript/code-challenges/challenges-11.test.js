...
const count = (target, input) => {
  ...
  num = input.reduce((accum, innerArr) => accum + innerArr.filter(element => element === target).length, 0);
  return num;
};
...
const totalSum = (input) => {
  let sum = 0;
  sum = input.reduce((accum, innerArr) => accum + innerArr.reduce((accum, element) => accum + element, 0), 0);
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let innerArr of input){
    result.push(innerArr.filter(element => (element % 5) === 0 && Number.isInteger(element)).map(element => Math.pow(2, element)));
  }
  return result;
};
...
let findMaleAndFemale = (data) => {
  let result = [];
  result = data.filter(element => element.gender === `male` || element.gender === `female`).map(element => element.name).join(' and ');
  return result;
};
...
let findShortest = (data) => {
  let character = '';
  character = data.reduce((accum, element) => (parseInt(element.height) < parseInt(accum.height))?element:accum).name;
  return character;
};
