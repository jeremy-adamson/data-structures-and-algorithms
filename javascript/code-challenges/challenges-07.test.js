...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++){
    ...
    ingredient = ingredient.slice(ingredient.indexOf(' ', ingredient.indexOf(' ') + 1) + 1);
...
const splitFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++){
    ...
    result.push(ingredient.slice(2).join(' '));
  }
  return result;
};
...
const totalSumCSV = (str) => {
  let total = 0;
  str.split(',').forEach(num => total = parseInt(num) + total);
  return total;
};
...
const extractVowels = (str) => {
  ...
  discard = discard.split('').sort().join('');
  return [remainder, discard];
};
