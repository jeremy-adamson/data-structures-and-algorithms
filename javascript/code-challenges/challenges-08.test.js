...
const sortByChildren = (charArray) => {
  return (charArray.sort((a, b) => {
    let test = a.children.length - b.children.length;
    if (test === 0) {
...
const citiesAtoJ = (arr) => {
  let pattern = new RegExp('^[A-J]+.*$');

  let result = arr.filter(element => pattern.test(element));
  return result;
...
const matchMonth = (input) => {
  let pattern = new RegExp('^[Oo]+ct+(ober)?$');
  return pattern.test(input);
