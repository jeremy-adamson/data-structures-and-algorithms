...
const returnNames = (arr) => {
  // let accum = [];
  // for (let i = 0; i < arr.length; i++){
  //   accum.push(arr[i].name);
  // }
  // return accum;
  return(arr.reduce((accum, element) => [...accum, element.name], []));
};
...
const countNumberOfChildren = (arr) => {
  return(arr.reduce((accum, person) => person.children ? accum + person.children.length : accum, 0));
};
...
const calculateAverage = (arr) => {
  // Not the best way to do it but the only one to make the return value of reduce itself to be the average
  return(arr.reduce((ave, number, i) => (ave * i + number)/(i+1.0),0));
};
...
const countPrimeNumbers = (arr) => {
  return(arr.reduce((num, element) => isPrime(element)? num + 1: num, 0));
};
...
const extractStats = (snorlaxData) => {
  return(snorlaxData.stats.reduce((accum, data) => {
    accum[data.stat.name] = data.baseStat;
    return accum;}, {}));
};
...
const extractChildren = (arr) => {
  return(arr.filter(name => name.name.includes('a')).reduce((accum, person) => person.children?[...accum, ...person.children]:accum, []));
};
