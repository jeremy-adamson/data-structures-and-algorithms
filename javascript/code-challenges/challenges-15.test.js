...
const screenForNames = (arr) => {
  let pattern = /^(Mr\. |Mrs\. |Ms\. |Dr\. )\w*$/m;
  let filtered = arr.filter(element => pattern.test(element));
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(element => {
    if(element.name === 'Luke Skywalker')
      compare = parseInt(element.mass);
  });
  let result = arr.filter(element => compare < parseInt(element.mass)).map(element => element.name);
...
const detectTicTacToeWin = (board) => {
  ...
  let diagValue = board[0][0]; //Stores the value of the first diagonal space for comparison
  let reverseDiagValue = board[0][board.length-1]; //Stores the value of the first reverse diagonal space for comparison
  let diagFlag = !!diagValue; //Flag to indicate if the diagonal may still contain a win
  let reverseDiagFlag = !!reverseDiagValue; //Flag to indicate if the reverse diagonal may still contain a win

  //Iterates to next row/column or to next diagonal space
  for (let i = 0; i < board.length; i++){
    let j = 0;
    let vertValue = board[i][j]; //Stores the value of the first space in that column comparison
    let horizValue = board[j][i]; //Store the value of the first space in that row for comparison
    let horizFlag = !!horizValue; //Flag to indicate if row may still contain a win
    let vertFlag = !!vertValue; //Flag to indicate if column may still contain a win

    //Iterate down each row/column
    while ((horizFlag || vertFlag) && j < board.length){

      //Compare to first value in that row/column to detect if win is still possible
      if(horizFlag)
        horizFlag = horizValue === board[j][i];
      if(vertFlag)
        vertFlag = vertValue === board[i][j];
      j++;
    }

    //Determine row/column win
    if (horizFlag || vertFlag){
      return true;
    }

    //Compare to first value in that diagonal/reverse diagonal to detect if win is still possible
    if(diagFlag)
      diagFlag = diagValue === board[i][i];
    if(reverseDiagFlag)
      reverseDiagFlag = reverseDiagValue === board[i][board.length-1-i];
  }

  //Determine diagonal win
  if (diagFlag || reverseDiagFlag)
    return true;

  //Base case, no win detected
  return false;
};
